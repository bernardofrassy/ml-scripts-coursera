...
def lr_param(dataX: pd.DataFrame, dataY: pd.DataFrame,
    ...
    #Gradient descent in case n is too large.
        ...
        w = grad_descent(X,Y, ls_cost, ls_cost_grad, alpha = alpha, lambdaFactor = lambdaFactor)
    return w
...
def ls_cost(X: np.matrix,Y: np.matrix,w: np.matrix,
    ...
    """
    n = X.shape[0]
    cost = 1/(2*n) * ((X * w - Y).T * (X * w - Y) + lambdaFactor * (w[1:].T * w[1:]))
    return cost
...
def grad_descent(X: np.matrix, Y: np.matrix,
    ...
    while ((error > 10**(-5)) and (count < maxIteration)):
        ...
        if costFunction(X,Y,wNew,lambdaFactor) > costFunction(X,Y,w,lambdaFactor):
