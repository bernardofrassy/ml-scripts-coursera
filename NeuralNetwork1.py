...
w, grad,neuron_list = neural_net_param(X_train,Y0_train,3,[3,3,3], alpha = 1)
prob_y0 = forward_propagation(X_cross,w)[-1]
pred_y0 = (prob_y0 > 0.5) * (1)
sum((pred_y0 == Y0_cross) * (1))/Y0_cross.shape[0]

w, grad,neuron_list = neural_net_param(X_train,Y1_train,3,[3,3,3], alpha = 1)
prob_y1 = forward_propagation(X_cross,w)[-1]
pred_y1 = (prob_y1 > 0.5) * (1)
sum((pred_y1 == Y1_cross) * (1))/Y1_cross.shape[0]

w, grad,neuron_list = neural_net_param(X_train,Y2_train,3,[3,3,3], alpha = 1)
prob_y2 = forward_propagation(X_cross,w)[-1]
